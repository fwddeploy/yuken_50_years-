...
export async function PUT(request: Request, context: { params: Promise<{ guestId: string }> }) {
  ...
  if (!await mayManageGuest(user, guestId)) { await recordRefusal(user.personId, "stay.change", "guest", guestId, "Guest is outside this coordinator's groups."); return Response.json({ error: "This guest is not in one of your guest groups." }, { status: 403 }); }
