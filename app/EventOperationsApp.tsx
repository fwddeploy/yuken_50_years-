  ...
  return <main className="loginPage">
    <section className="loginShell" aria-label="Yuken India Golden Jubilee event team sign in">
      <div className="loginArtwork">
        {/* Same cover photo as before, cropped to trim the dead space above the
            panel (39KB WebP, no runtime optimiser needed) — object-fit:contain
            lets the artwork panel's own background provide even top/bottom
            margin instead of the photo's own pixels ending at zero margin. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/yuken-50-badge.webp" alt="Yuken India Limited — 50 years, five decades of friendly and intelligent service" fetchPriority="high" decoding="async" />
      </div>
...
function UpdatesView({ jobs, query, ownerFilter, openUpdate }: { jobs: EventJob[]; query: string; ownerFilter: string | null; openUpdate: (jobId: string, updateId: string) => void }) {
  ...
  const [venueFilter, setVenueFilter] = useState<"malur" | "taj">("malur");
  const [dayOpen, setDayOpen] = useState(false);
  const [personOpen, setPersonOpen] = useState(false);
  const authors = [...new Set(jobs.flatMap(job => job.updates.map(update => update.author)))].sort();
  const term = query.trim().toLocaleLowerCase("en-IN");
  const updates = jobs.filter(job => !ownerFilter || job.ownerIds.includes(ownerFilter)).flatMap(job => job.updates.map(update => ({ ...update, job }))).filter(update => (person === "all" || update.author === person) && (update.job.venue === venueFilter || update.job.venue === "general") && matchesUpdateDay(update.at, day) && (!term || `${update.author} ${update.message} ${update.job.title}`.toLocaleLowerCase("en-IN").includes(term)));
  ...
  return <><section className="guestHero staysHero updatesHero"><div><p className="eyebrow">Across the committee</p><h1>Team updates</h1><p>What everyone has done, newest first. Tap an update to open the activity behind it.</p></div></section>
    <div className="eventSwitch sectionEventSwitch" role="group" aria-label="Choose the evening">
      <button className={venueFilter === "malur" ? "active" : ""} onClick={() => setVenueFilter("malur")}><b>15</b><span>November<small>YIL Malur</small></span></button>
      <button className={venueFilter === "taj" ? "active" : ""} onClick={() => setVenueFilter("taj")}><b>18</b><span>November<small>Taj West End</small></span></button>
    </div>
