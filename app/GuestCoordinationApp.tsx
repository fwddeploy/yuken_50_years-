...
function MineView({ user, snapshot, send, saveAgenda, saveTravel }: { user: EventUser; snapshot: GuestSnapshot; send: (audience: SendAudience) => void; saveAgenda: (groupId: string, date: string, items: GuestAgendaItem[]) => Promise<boolean>; saveTravel: (plan: GuestTravelPlan) => Promise<boolean> }) {
  ...
  return <>
    ...
    {allDays ? <>
      ...
      <MessagePreview title="The agenda lines you are sending" text={agendaMessage} note="These lines go inside the approved template for each guest's language. Tap Send to see the exact message before anything goes out." />
      ...
      <div className="mineTravelList">{dayTravel.map(plan => <button key={plan.id} onClick={() => { if (user.isCore) setEditingTravel(plan); }}><span className="travelMode">{plan.mode}</span><span><b>{plan.name}</b><small>{plan.stops[0] ? `${formatTime(plan.stops[0].time)} · ${plan.stops[0].place}` : plan.routeName}</small></span><i>›</i></button>)}</div>
      ...
      {user.isCore ? <button className="secondaryAction mineAddLine" onClick={addTravel}>＋ Add travel for this group</button> : <p className="mineAllNote">Vehicles are arranged by the transport team. You can see them here and send the details to your group.</p>}
      <MessagePreview title="The travel details you are sending" text={travelMessage} note="Each guest receives only their own vehicle, inside the approved template. Tap Send to see the exact message before anything goes out." />
...
function MessagePreview({ title, text, note }: { title: string; text: string; note: string }) {
  return <section className="messagePreview"><header><b>{title}</b><span>Your content</span></header><textarea value={text} readOnly aria-label={title} /><p>{note}</p></section>;
}
...
function InvitationsView({ user, preview, snapshot, send, notify }: { user: EventUser; preview: boolean; snapshot: GuestSnapshot; send: (audience: SendAudience) => void; notify: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="updatesPickers"><FilterPicker label="Showing" value={showing} options={showingOptions} change={setShowing} />{showing !== "everyone" && <button className="updatesClear" onClick={() => setShowing("everyone")}>Back to everyone</button>}</div>
...
function TravelView({ user, snapshot, save, send }: { user: EventUser; snapshot: GuestSnapshot; save: (plan: GuestTravelPlan) => Promise<boolean>; send: (audience: SendAudience) => void }) {
  ...
  return <>
    ...
    <EventSwitch event={event} change={setEvent} />
    <section className="guestToolbar"><SearchBox value={query} change={setQuery} placeholder="Search route or category" /><div className="plainRule"><b>Category-based travel</b><span>Every guest in a selected category receives this route. Individual guest exceptions are not used.</span></div>{user.isCore && <button className="secondaryAction addGuestAction" onClick={addPlan}>＋ Add travel plan</button>}</section>
    <div className="sectionTitle prototypeSectionTitle"><div><p className="eyebrow">Shared routes and vehicles</p><h2>Travel plans</h2></div><span>{plans.length} plans</span></div>
...
function SendSheet({ audience, snapshot, preview, close, notify }: { audience: SendAudience; snapshot: GuestSnapshot; preview: boolean; close: () => void; notify: (message: string) => void }) {
  ...
  const [review, setReview] = useState<{ batchId: string; ready: number; skipped: number; sample: { guestName: string; subject: string | null; body: string } | null; notSent: { guestId: string; guestName: string; reason?: string; missing?: string[] }[] } | null>(null);
  ...
  async function reviewMessages() {
    ...
    try {
      ...
      const payload = await response.json() as { error?: string; batchId?: string; ready?: number; skipped?: number; sample?: { guestName: string; subject: string | null; body: string } | null; notSent?: { guestId: string; guestName: string; reason?: string; missing?: string[] }[] };
      ...
      setReview({ batchId: payload.batchId, ready: payload.ready ?? 0, skipped: payload.skipped ?? 0, sample: payload.sample ?? null, notSent: payload.notSent ?? [] });
