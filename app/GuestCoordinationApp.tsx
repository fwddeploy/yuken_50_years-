...
export default function GuestCoordinationApp({ user, back, signOut }: { user: EventUser; back: () => void; signOut: () => void }) {
  ...
  async function saveTravel(next: GuestTravelPlan) {
    if (preview) {
      setSnapshot(value => ({ ...value, travelPlans: value.travelPlans.map(plan => plan.id === next.id ? next : plan) }));
      setToast("Travel plan saved in this local preview."); return;
    }
    try {
      const response = await fetch(`/api/guest/travel/${encodeURIComponent(next.id)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event: next.event, date: next.date, mode: next.mode, routeName: next.routeName, vehicleNumber: next.vehicleNumber, driverName: next.driverName, driverPhone: next.driverPhone, categoryIds: next.categories.map(category => category.id), stops: next.stops }) });
      const payload = await response.json() as { error?: string };
  ...
  return <main className="eventApp guestApp">
    ...
    <div className="eventBody guestBody">
      {loading ? <GuestLoading /> : <>
        ...
        {tab === "guests" && <GuestsView snapshot={snapshot} save={saveGuest} />}
...
function GuestsView({ snapshot, save }: { snapshot: GuestSnapshot; save: (guest: GuestRecord) => void }) {
  const [event, setEvent] = useState<GuestEvent>("malur");
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState("all");
  ...
  return <>
    ...
    {selected && <GuestEditSheet guest={selected === "new" ? undefined : selected} event={event} snapshot={snapshot} close={() => setSelected(null)} save={guest => { save(guest); setSelected(null); }} />}
  </>;
}
...
function TravelView({ snapshot, save }: { snapshot: GuestSnapshot; save: (plan: GuestTravelPlan) => void | Promise<void> }) {
  ...
  return <>
    <EventSwitch event={event} change={setEvent} />
    <section className="guestToolbar"><SearchBox value={query} change={setQuery} placeholder="Search route or category" /><div className="plainRule"><b>Category-based travel</b><span>Every guest in a selected category receives this route. Individual guest exceptions are not used.</span></div></section>
...
function GuestEditSheet({ guest, event, snapshot, close, save }: { guest?: GuestRecord; event: GuestEvent; snapshot: GuestSnapshot; close: () => void; save: (guest: GuestRecord) => void }) {
...
function TravelSheet({ plan, categories, close, save }: { plan: GuestTravelPlan; categories: GuestSnapshot["categories"]; close: () => void; save: (plan: GuestTravelPlan) => void }) {
  const [mode, setMode] = useState(plan.mode), [date, setDate] = useState(plan.date), [routeName, setRouteName] = useState(plan.routeName);
  ...
  function submit(event: FormEvent) {
    ...
    save({ ...plan, date, mode: mode.trim(), routeName: routeName.trim(), vehicleNumber: vehicleNumber.trim() || undefined, driverName: driverName.trim() || undefined, driverPhone: driverPhone.trim() || undefined, categories: categories.filter(category => categoryIds.has(category.id)), stops: stops.map((stop, index) => ({ ...stop, order: index + 1 })) });
  }
  return <Sheet title="Edit travel plan" subtitle={plan.name} close={close}><form className="recordForm travelForm" onSubmit={submit}><div className="formPair"><label><span>Travel date</span><input type="date" value={date} onChange={event => setDate(event.target.value)} required /></label><label><span>Travel mode</span><input value={mode} onChange={event => setMode(event.target.value)} placeholder="Coach, car…" required /></label></div><label><span>Route name</span><input value={routeName} onChange={event => setRouteName(event.target.value)} required /></label><div className="formPair"><label><span>Vehicle number</span><input value={vehicleNumber} onChange={event => setVehicleNumber(event.target.value)} placeholder="Add when confirmed" /></label><label><span>Driver name</span><input value={driverName} onChange={event => setDriverName(event.target.value)} placeholder="Add when confirmed" /></label></div><label><span>Driver phone</span><input value={driverPhone} onChange={event => setDriverPhone(event.target.value)} inputMode="tel" placeholder="Add when confirmed" /></label><fieldset className="categoryChecks"><legend>Guest categories using this travel plan</legend>{categories.map(category => <label key={category.id}><input type="checkbox" checked={categoryIds.has(category.id)} onChange={() => toggleCategory(category.id)} /><span>{category.name}</span></label>)}</fieldset><div className="stopEditor"><div className="sectionTitle guestSectionTitle compact"><div><p className="eyebrow">In travel order</p><h2>Stops</h2></div></div>{stops.map((stop, index) => <div className="stopEditRow" key={stop.id}><span className="stopNumber">{index + 1}</span><label><span>Time</span><input type="time" value={stop.time} onChange={event => setStops(value => value.map(row => row.id === stop.id ? { ...row, time: event.target.value } : row))} required /></label><label><span>Place</span><input value={stop.place} onChange={event => setStops(value => value.map(row => row.id === stop.id ? { ...row, place: event.target.value } : row))} required /></label><button type="button" aria-label={`Remove stop ${index + 1}`} onClick={() => setStops(value => value.filter(row => row.id !== stop.id))}>×</button></div>)}<button type="button" className="secondaryAction addLineAction" onClick={() => setStops(value => [...value, { id: crypto.randomUUID(), order: value.length + 1, time: "09:00", place: "" }])}>＋ Add stop</button></div><button className="primaryAction" disabled={!categoryIds.size || !stops.length || stops.some(stop => !stop.time || !stop.place.trim())}>Save travel plan</button></form></Sheet>;
}
