...
/** An address the mail provider can actually deliver to. A non-empty string
 *  was previously enough, so "ask reception" counted as reachable and failed
 *  one recipient at a time after the send was confirmed. */
export function sendableEmail(value: string | null | undefined): string | null {
  const cleaned = (value ?? "").trim();
  return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/u.test(cleaned) ? cleaned : null;
}
...
export function preflightMessages(input: { purpose: MessagePurpose; channel: MessageChannel; templates: MessageTemplate[]; recipients: MessageRecipient[] }) {
  const items: MessagePreflightItem[] = input.recipients.map(recipient => {
    const contact = input.channel === "whatsapp" ? sendablePhone(recipient.phone) : sendableEmail(recipient.email);
